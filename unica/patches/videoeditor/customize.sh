...
EVAL "echo \"ro.studio.soc.model        u:object_r:soc_prop:s0 exact string\" >> \"$WORK_DIR/system/system/etc/selinux/plat_property_contexts\""

if [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "qssi" ]]; then
    SET_PROP "system" "ro.studio.soc.model" "sm7125"
elif [[ "$TARGET_OS_SINGLE_SYSTEM_IMAGE" == "essi" ]]; then
    SET_PROP "system" "ro.studio.soc.model" "exynos9611"
fi
